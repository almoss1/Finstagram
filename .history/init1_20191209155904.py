...
def analytics():
    ...
    query = 'SELECT photoID, photoPoster, total_rating FROM (SELECT photoID, photoPoster, SUM(rating) AS total_rating FROM photo NATURAL JOIN likes WHERE photoPoster= %s group by photoPoster, photoID) AS T'
    cursor.execute(query, (user))
