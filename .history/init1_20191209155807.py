...
def analytics():
    ...
    query = 'SELECT photoID, photoPoster, total_rating FROM (SELECT photoID, photoPoster, SUM(rating) AS total_rating FROM photo NATURAL JOIN likes WHERE photoPoster= %s group by photoPoster, photoID) AS T HAVING total_rating=MAX(T.total_rating)'
    cursor.execute(query, (user))
