...
def analytics():
    ...
    query = 'SELECT photoID, photoPoster, SUM(rating) AS total_rating FROM photo NATURAL JOIN likes GROUP BY photoPoster, photoID HAVING SUM(rating)=(SELECT MAX(max_value) FROM (SELECT SUM(rating) AS max_value FROM photo NATURAL JOIN likes WHERE photoPoster= %s group by photoPoster, photoID))'
    cursor.execute(query, (user))
