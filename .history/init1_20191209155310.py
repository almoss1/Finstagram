...
def analytics():
    ...
    query = 'SELECT photoID, photoPoster, MAX(total_rating) AS total_rating FROM (SELECT photoID, photoPoster, SUM(rating) AS total_rating FROM photo NATURAL JOIN likes WHERE photoPoster= %s group by photoPoster, photoID) WHERE photoPoster= %s'
    cursor.execute(query, (user))
