...
def show_photo(currPhotoID):
    ...
    query = 'SELECT * FROM like WHERE photoID=%s AND username=%s'
    cursor.execute(query, (currPhotoID, user))
    liked = cursor.fetchone()

    cursor.close()
    return render_template('show_photo.html', post=data, tagged=taggees, likees=likes, owner=owner, isLiked=liked)
